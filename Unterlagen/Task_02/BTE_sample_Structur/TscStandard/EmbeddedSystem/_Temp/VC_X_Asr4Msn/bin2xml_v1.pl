...
my $testcase_str = chr(hex '66').chr(0).chr(0).chr(0);
# folgende Strings werden momentan nicht gebraucht
...
{	
	...
	# testcase check
	if(substr($zeilen_binary,$i,$data_length) eq $testcase_str) # falls naechste 8 Bytes dem testcase byte entsprechen
	{
		$testcase_id = substr($zeilen_binary,$i+$data_length,$data_length);	# read Bytes (Bitnotation)
		my $testcaseid_help = unpack "I<", "$testcase_id";	# convert into char		
		
		if($controlflow_testcase == 0){ # tastcase Initialisierung
			print "$str_testcase1$testcaseid_help\"\>\n";
			print "$str_testrun1\n";
			print "$str_log\n";
			print SCHREIBEN "$str_testcase1$testcaseid_help\"\>\n";
	...
	{		
		...
		if ($numParam eq chr('0'))	# falls kein Parameter vorhanden
		{
		...
		elsif ($numParam eq chr('1'))
		{
			...
			my $substring_help = substr($zeilen_binary,$i+(2*$data_length),1*2*$data_length);	# 2 length of data struct, 4 numParam
			my $c_h = $data_length;
			...
			# hiernach muss numParam*2*data_length gewartet werden, damit keine Variablen ohne Grund ueberschrieben werden
			$i = $i + (1*2*$data_length);
		...
		elsif ($numParam eq chr('2'))
		{
			...
			my $substring_help = substr($zeilen_binary,$i+(2*$data_length),2*2*$data_length);	# 2 length of data struct, 4 numParam
			my $c_h = $data_length;
			$data1 = substr($substring_help,$c_h,$data_length);
			my $string_h=unpack "H*", $data1;
			print SCHREIBEN "$str_elem1$type_help$str_elem2$time_help$str_elem3$id_help data1=$string_h ";
		...
		elsif ($numParam eq chr('3'))
		{
			...
			my $substring_help = substr($zeilen_binary,$i+(2*$data_length),3*2*$data_length);	# 2 length of data struct, 4 numParam
			my $c_h = $data_length;
			$data1 = substr($substring_help,$c_h,$data_length);
			my $string_h=unpack "H*", $data1;
			print SCHREIBEN "$str_elem1$type_help$str_elem2$time_help$str_elem3$id_help data1=$string_h ";
		...
		{
			my $substring_help = substr($zeilen_binary,$i+(2*$data_length),5*2*$data_length);	# 2 length of data struct, 4 numParam
			my $c_h = $data_length;
			$data1 = substr($substring_help,$c_h,$data_length);
			my $string_h=unpack "H*", $data1;
			print SCHREIBEN "$str_elem1$type_help$str_elem2$time_help$str_elem3$id_help data1=$string_h ";
			...
			print "data4=$string_h$str_elem_e\n";
			$c_h = $c_h + 8;
			$data5 = substr($substring_help,$c_h,$data_length);
			$string_h=unpack "H*", $data5;
			print SCHREIBEN "data5=$string_h$str_elem_e\n";
			print "data5=$string_h$str_elem_e\n";
