...
{
  char tempText[100];
  (void)sprintf(tempText, "TC:\"%u\"", id );
  BteLogList_AddTestcase( id );
  BteTestSequence_Begin(tempText);  
}
...
void BteTestCase_SetState( tBteTestCaseState state )
{
  ...
  case BTETESTCASE_STARTTESTCASE:
    BteTestSequence_BeginID(bteTestSequence.id_num);
    break;
    /*----------------------------------------------------------------------*/
...
void BteTestHandler_BackgroundTest_Log(void)
{
  ...
  BteTestSequence_BeginID(1); // Der Anwender soll diese Methode verwenden
#if defined (USE_EXTERNAL_LOG)
  for(i=0; i<kBteBgrTest_MaxCount; i++)
  {
