...
static void TscDet_Event_Log( tBteEvent *pEvent, const char* text_name, const char* text_param);
...
FUNC(void, DET_CODE) Det_ReportError( uint16 ModuleId, uint8 InstanceId, uint8 ApiId, uint8 ErrorId )
{
  ...
  evnt.data1 = (uint32)ModuleId<<24 | InstanceId<<16 | ApiId<<8 | ErrorId;
  ...
  //TscDet_Event_Log( &evnt  , "Det_ReportError", "(ModuleId=%d, InstanceId=%d, ApiId=%d, ErrorId=%d)" );
