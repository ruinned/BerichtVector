...
void Dio_WriteChannel(Dio_ChannelType DioChannel, Dio_LevelType DioLevel)
{
  if( STD_HIGH == DioLevel )
  {
    if( DioChannel == Dio_FrTrcv_EN_A )
    {
      ...
      ArCom_DioWriteChannel(0x05, 0x01);
#endif
    }
    else
...
#if defined(ARDUINO) 
        ArCom_DioWriteChannel(0x03, 0x01);
#endif
      }
  }
  else
  {
    if( DioChannel == Dio_FrTrcv_EN_A )
    {
      /* Set EN Pin to low */
#if defined(ARDUINO) 
      ArCom_DioWriteChannel(0x05, 0x00);
#endif
    }
    else
      if( DioChannel == Dio_FrTrcv_STBN_A )
      {
        /* Set STBN Pin to low */
#if defined(ARDUINO) 
        ArCom_DioWriteChannel(0x03, 0x00);
#endif
      }
  }
...
void Dio_SetTransceiverStatusWord(uint8 reg, uint8 shift, uint16 sw)
{
  ...
  if(reg == 0){
    ...
    switch(sw){
    case 0x0800u:// FRTRCV_30_TJA1080_LOCALWAKEUP
      ArdCom_ComplexOp_WakeUpPattern(5);
      break;
    case 0x0100u:// FRTRCV_30_TJA1080_BUSERROR
      ArdCom_ComplexOp_ForceBusCollision();
      //ArdCom_WriteReadTRCVStatusWord(TRCV_MODE,&swlocal);
      break;
