...
Vektor und somit die Abteilung PES sind aus diesem Grund darauf angewiesen, in naher Zukunft einen Umstieg in die neue Version zu ermöglichen. Da dies sich nicht ohne Weiteres umsetzen lässt, muss dazu eine detaillierte Analyse vor allem über die Kompatibilität zwischen den beiden Versionen durchgeführt werden.
%
...
Um mir einen Überblick über die Verwendung der betrachteten Software zu verschaffen, war es nötig eine statische Analyse einer BSW-Komponente durchzuführen. Die Analyse erfolgte mithilfe der Version 7.0. Das entsprechende Ergebnis ist in Abbildung \ref{fig:QAC7Single} dargestellt.
%
\begin{figure}[!htp]
\centering
...
In Abbildung \ref{fig:QACfunc} kann man erkennen, dass beim Teil \textit{Configuration} bestimmte Files notwendig sind, damit ein QA-C-Projekt richtig initialisiert und überhaupt die Analyse stattfinden kann. Diese Einstellungsdateien werden als \textit{personalities} bezeichnet.

Nach~\cite{QAC70WinUserGuide} weisen die jeweiligen Dateien folgende Eigenschaften:
...
Inerhalb der Datei $*.p_c$ musste ich die Definition $\_WIN32 \quad \_WINNT$ anpassen, da auf dem Computer, auf dem die statische Codeanalyse durchgeführt werden soll, ein 64-bit-Betriebssystem läuft. Somit sollen manche Makros angepasst oder sogar außer Betracht gelassen werden .
%
\begin{figure}[!htp]
\centering
...
\end{figure}
%
\paragraph*{\uppercase{Lösungsansatz zum Vergleich der relevanten MISRA-Versionen}}
%
Die Version des genannten Analysewerkzeugs, für das Vector über eine Lizenz verfügt, ist die 7.0. Diese Version unterstützt die Untersuchung von Software-Produkten darauf, ob diese mit dem MISRA-C:2004-Standard konform gehen. 

Für einen möglichen Umstieg auf MISRA 2012 sind somit die Unterschiede der beiden betrachteten MISRA Standards wie oben erläutert genauer zu analysisieren und gegenüberzustellen. Es ist jedoch nicht nur der obiger Vergleich nötig, dazu sind die QA-C Versionen 7.0 und 9.0 in Bezug auf unterstützte Regeln und Toolnutzung miteinander zu vergleichen.


Nachdem die richtigen Einstellungen für ein QAC-Projekt ausgewählt und vorgenommen wurden, haben die ersten Analysen stattgefunden, welche Hinweise auf eventuell fehlerhafte Teile beim Code ausgaben. Die Analyse im nächsten Unterkapitel.
%
