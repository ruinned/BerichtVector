...
\subsection{Arbeitsbericht}
%
In der sechsten Woche wurde mir die 2. Aufgabestellung vorgestellt. Dabei ginge es darum, mich umfassend mit einem Basic Test Environment (BTE) zu beschäftigen, welches in der Vector GmbH entwickelt wurde. Ich musste mich mit dem Tool vertraut machen und verstehen wie es funktioniert. Ziel dabei war es, das Tool um weitere Funtionalitäten zu erweitern.
%
\subsubsection{Zweite Aufgabenstellung}
% 
Erstens musste ich das Tool verstehen und in der Lage sein, seine Funktionalitäten zu beschreiben. Aus diesem Grund werden im Folgenden einige seiner Eigenschaften und Besonderheiten zusammengefasst und anschliessend die zweite Aufgabe meiner Tätigkeit festgelegt.
%
\subsubsection*{BTE - Basic Test Environment}
%
Das Basic Test Environment (BTE) ist ein Component-Unit-Test-Framework, mit dessen Hilfe man in Vector hardwareunabhängige (AUTOSAR-) BSW-Komponeneten testet \cite{BTE}.
...
Im Diagram wird durch die rote Box die Nachbildung der inneren ECU-Umgebung gezeigt. Diese Box besteht aus mehreren Platzhaltern, sogenannten Stubs, die Komponenten simulieren/ersetzen. Dabei ist ersichtlich, dass der Testplan sowohl die simulierten Module als auch die Component-Under-Test steuert, damit anschliessend vom Framework einen entsprechenden Log/Report ausgegeben wird.

Das Klassendiagram, welches in Abbildung \ref{fig:BTELogListold} gezeigt wird, gibt einen Teil der BTE-Struktur wieder. Dabei sind diejenige der Komponenten gezeigt, welche sich um die Erstellung eines Testreports kümmern. Das Teilmodul \verb+TestHandler+ steuert den Ablauf der Testdurchführung und die Erstellung des Testreports über das Modul \verb+ReportHandler+. 
%
\begin{figure}[!htp]
\centering
\includegraphics[scale=0.7]{./Bilder/BTELogListold.pdf}
\caption{BTEUseCase}
\label{fig:BTELogListold}
\end{figure}
%

Gegenüber den genannten Funktionalitäten gibt es bei der Verwendung des BTEs folgenden Nachteil, der sich eher als eine Einschränkung äußert: Mit dem Tool lassen sich zwar bequem Testreports auf einem lokalen Rechner ausgeben und abspeichern, betriebssystemabhängige Funktionen u.a. zum Umgang mit Strings werden jedoch dabei verwendet. Diese Funktionalitäten benötigen somit viele Hardwareresourcen. 

Es wäre somit auch vorteilhalft, die BTE-Struktur so zu erweitern, dass diese ebenfalls auf einem beliebigen bzw. eingebetteten System laufen. Die Komponententests (Component Unit Test) könnten dadurch unter realen bzw. Echtzeitbedingungen durchgeführt werden.

Die Aufgabestellung basiert somit auf der beschriebenen Anforderung, das BTE-Framework so zu ergänzen, dass dieses während einer beliebigen Testdurchführung gleichzeitig mit der CUT\footnote{Component Under Test} auf dem eingebetteten System läuft. 

Man muss dementsprechend mit den vorhandenen Hardwareressourcen des betrachteten eingebetteten Systems sparsam umgehen und dabei auch eine Methode einsetzen, bei der beispielsweise der Test-Report direkt auf dem beschränkten RAM-Speicher abgelegt wird. Dazu ist es notwendig, eine bestimmte Codierung festzulegen, wie die Daten abzuspeichern sind.

Am Ende der Testdurchführung soll der Test-Report wieder als eine XML-Datei vorliegen. Dies ist nur dann möglich, wenn die entsprechenden Stellen des RAM-Speichers aus dem eingebetteten System extrahiert und anschließend analysiert werden. Dieser RAM-Bereich, welcher als eine Binärdatei vorliegt, würde sich mithilfe einer Anwendung analysieren lassen. Diese Anwendung wird in einer ausgewählten Skriptsprache programmiert und implementiert die Dekodierung der Daten so, dass erneut ein Test-Report als eine XML-Datei erstellt werden kann.

Die Umsetzung der genannten Ansätze sind in \secref{durchfAuf2} erklärt.
%
...
In ~\cite{STM32Eclipse} bzw.~\cite{EclipseSTM32Youtube} werden weitere, notwendige Tools angegeben, die die Programmierung des Mikrocontrollers ermöglichen:
%
\begin{itemize}
...
\subsection{Arbeitsbericht}
%
\subsubsection{Anforderungsanalyse}\label{Anford}
%
Das Tool, welches auf der Erweiterung der Funktionalitäten der BTE-Umgebung und Implementierung eines PERL-Skripts zur Erstellung des Testreports aufbaut, soll folgende Anforderungen erfüllen:
%
\begin{itemize}
\item Ursprüngliche BTE-Funktionalitäten, einen Testreport zu strukturieren und erzeugen, sollen durch Erweiterungen möglichst wenig modifiziert werden.
...
\item Es soll durch eine entsprechende Kodierung Testreports auf der RAM eines beliebigen Embedded-Prozessors abspeichern können.
\item Das Tool soll aus einer erzeugten Binärdatei, welche das Memory Mapping eines RAM-Speicherbereichs darstellt, einen Testreport erstellen können.
\end{itemize}
%
...
\begin{figure}[!htp]
\centering
\includegraphics[scale=0.7]{./Bilder/BTELogList.pdf}
\caption{BTEUseCase}
\label{fig:BTELogList}
\end{figure}
\end{landscape}
%
%
...
\begin{itemize}
\item Man bindet entsprechende Ersatzbibliotheken ein, die optimiert und ressouerce-sparend arbeiten.
...
Die zweite Möglichkeit wird zur Lösung der Aufgabe umsetzt. Dabei wird wie folgt vorgegangen:

Mit vorhandenen und zusätzlich eingeführten Präprozessor-Direktiven (\verb+#define+) wird die Möglichkeit geboten, die BTE-Methodensaufrufe auszublenden, die zu viele Rechenressourcen benötigen, um richtig zu funktionieren. Ebenfalls sollten solche Methodensaufrufe ausfallen, die auf der eingebetteten Hardware nicht kompilierbar sind, da diese Hardwareressourcen verwenden, die dort nicht zur Verfügung stehen. Ein Beispiel dazu sind solche Methoden, die das lokale Abspeichern von Dateien ermöglichen wie fopen, fclose, sprintf, usw. 

Mithilfe der neu eingeführten Direktive \verb+#define USE_PRINTF+ werden alle Methodensaufrufe ausgeblendet, die lediglich mit Strings umgehen. Dies ermöglicht somit solche Methoden nur dann einzusetzen, wenn die Hardware genug RAM zur Verfügung stellt und dort die notige Funktion implementiert ist.
%
...
Durch das Umdefinieren einer schon vorhandenen Direktive \verb+#define BTE_ENABLE_TESTREPORT+ wird ein Ausblenden der oben genannten Methodensaufrufe möglich, die mit dem Erstellen, der Bearbeitung und lokalen Abspeicherung von Dateiobjekten umgehen. Siehe dazu den folgenden Codeabschnitt.
...
\verb+#define USE_HARDWARE+ wurde ebenfalls eingeführt, um bestimmte Codeabschnitte einzublenden wenn die BTE innerhalb des eingebetteten Systems verwendet wird und die hardwareabhängige bzw. ECU-eigene Funktionsaufrufe einzublenden. .
%
...
%
\begin{itemize}
\item Erste Datenstruktur
\item Zweite Datenstruktur
\end{itemize}
%
Die erste Struktur hat folgende Strukturen als Haupteigenschaft:
%
\begin{lstlisting}[style=C_colored_smallfont, caption = {Ausschnitt aus TscTest.c}, label={lst:usehardware}]
